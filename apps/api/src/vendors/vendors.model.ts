...
export class Vendor {
  ...
  @prop({ required: true, enum: ['plumbing','electrical','carpentry','painting','cleaning','security','landscaping','general'] }) specialty!: VendorSpecialty;
  @prop({ required: true, enum: ['active','inactive','suspended'], default: 'active' }) status!: VendorStatus;
  @prop() address?: string;
