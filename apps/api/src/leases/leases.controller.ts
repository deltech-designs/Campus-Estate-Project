...
import type { UpdateLeaseDto } from './dtos/update-lease.dto';
...
export const createLease = async (req: AuthRequest, res: Response): Promise<void> => { const data = await service.createLease(req.body as CreateLeaseDto); sendSuccess(res, data, 'Lease created', 201); };
export const updateLease = async (req: AuthRequest, res: Response): Promise<void> => { const data = await service.updateLease(req.params['id'] as string, req.body as UpdateLeaseDto); sendSuccess(res, data, 'Lease updated'); };
