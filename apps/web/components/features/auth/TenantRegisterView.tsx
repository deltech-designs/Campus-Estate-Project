...
import { Home, ShieldCheck, CreditCard, Wrench, ArrowRight, User } from 'lucide-react';
...
export function TenantRegisterView() {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      setError(null);
      const res = await authRegister(values.firstName, values.lastName, values.email, values.password, 'tenant', values.phone);
      if (res.otpRequired) {
  ...
  const handleGoogleSelect = async (account: { name: string; email: string }) => {
    ...
    try {
      ...
      await authRegister(firstName, lastName, account.email, 'GoogleOAuthPassword123!', 'tenant');
      router.push('/overview');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[var(--color-surface)] flex flex-col md:flex-row">
      ...
      <div className="hidden md:flex md:w-[45%] bg-[var(--color-sidebar-bg)] p-8 md:p-12 flex-col justify-between text-white relative overflow-hidden">
        {/* Decorative Glow */}
        <div className="absolute -left-20 -top-20 w-80 h-80 rounded-full bg-emerald-500/10 blur-3xl animate-pulse-subtle" />
        <div className="absolute -right-20 -bottom-20 w-80 h-80 rounded-full bg-emerald-600/5 blur-3xl animate-float-delayed" />

        {/* Brand / Logo */}
        <div className="relative z-10">
          <Link href="/" className="inline-flex items-center gap-3 group">
            <div className="w-10 h-10 rounded-xl bg-emerald-500 flex items-center justify-center font-bold text-lg text-white shadow-md shadow-emerald-500/20 group-hover:scale-105 transition-transform duration-300">
              E
            </div>
            <span className="font-[var(--font-display)] font-bold text-lg tracking-tight hover:text-emerald-400 transition-colors">
              CampusEstate
            </span>
          </Link>
        </div>
        ...
        <div className="my-auto py-12 relative z-10 max-w-md">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 mb-6">
            <User size={12} /> For Students & Faculty
          </span>
          <h2 className="text-3xl md:text-4xl font-extrabold font-[var(--font-display)] tracking-tight leading-tight mb-4">
            Find your perfect <span className="text-emerald-400">campus space</span> with ease.
          </h2>
          <p className="text-sm text-[var(--color-sidebar-text)] leading-relaxed mb-8">
            Access verified accommodations, manage digital lease agreements, pay rent securely, and log maintenance reports in seconds.
          </p>
          ...
          <div className="space-y-5">
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="text-sm font-semibold">Verified Hostel Postings</p>
                <p className="text-xs text-[var(--color-sidebar-text)] mt-0.5">Explore properties near your campus vetted for safety and quality.</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="text-sm font-semibold">Instant Rent Receipts</p>
                <p className="text-xs text-[var(--color-sidebar-text)] mt-0.5">Make secure digital payments and access billing statements immediately.</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <p className="text-sm font-semibold">Electronic Maintenance Logs</p>
                <p className="text-xs text-[var(--color-sidebar-text)] mt-0.5">Report faults on your phone and get real-time progress updates.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="w-full max-w-md">
          ...
          <div className="mb-8">
            ...
            <p className="text-xs text-[var(--color-text-secondary)] mt-1.5">
              Fill in your details to start exploring campus accommodations.
            </p>
          </div>
            ...
            {otpRequired ? (
              <form onSubmit={handleVerifyOtp} className="flex flex-col gap-5">
                ...
                <Input
                  ...
                  className="bg-[var(--color-surface-sunken)] border-[var(--color-border)] focus:ring-emerald-500/25 py-2.5 rounded-[var(--radius-md)] text-center tracking-[0.5em] text-lg font-bold"
                />

                <Button
                  type="submit"
                  loading={otpVerifying}
                  className="mt-2 w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold rounded-[var(--radius-btn)] transition-all duration-150"
                >
                  Verify Code
                </Button>
              ...
              <>
                ...
                <Button
                  ...
                  className="w-full flex items-center justify-center gap-3 py-3 border border-[var(--color-border)] bg-[var(--color-surface)] hover:bg-[var(--color-surface-sunken)] hover:border-emerald-500/20 text-[var(--color-text-primary)] font-semibold rounded-[var(--radius-btn)] transition-all duration-150"
                >
                  {!googleLoading && (
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4" noValidate>
                  <div className="grid grid-cols-2 gap-4">
                    <Input
                      ...
                      {...register('firstName')}
                      className="bg-[var(--color-surface-sunken)] border-[var(--color-border)] focus:ring-emerald-500/25"
                    />
                    <Input
                      label="Last name"
                      placeholder="Doe"
                  ...
                  <Button
                    ...
                    className="mt-2 w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white shadow-md shadow-emerald-600/10 font-semibold rounded-[var(--radius-btn)]"
                  >
                    Sign Up
                  </Button>
                </form>
